...
import com.lagou.educourseboot.service.OrderRemoteService;
...
public class CourseServiceImpl implements CourseService {
        ...
        if(null == courseDTOS){
            synchronized (this) {
                ...
                if(null == courseDTOS){
                    ...
                    // 查询全部课程
                    List<Course> courses = getInitCourses(null);
    ...
    @Override
    public List<CourseDTO> getMyCoursesByUserId(Integer userid) {
        List<Object> ids = orderRemoteService.getOKOrderCourseIds(userid);
        return getAllCourse(ids);
    }
//查询已购买的课程
    @Override
        ...
        if(null == courseDTOS){
            synchronized (this) {
                ...
                if(null == courseDTOS){
                    System.out.println("===查询mysql===");
                    // 查询全部课程
                    List<Course> courses = getInitCourses(ids);
                    courseDTOS = new ArrayList<>();
                    for(Course course : courses){
                        CourseDTO dto = new CourseDTO();
                        // course将属性全部赋给给courseDTO对象
